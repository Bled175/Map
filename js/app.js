...
function loadconnections() {
  ...
  connections.forEach(connection => {
    const fromPin = pins.find(
      p => p.id === connection.from
    )
    const toPin = pins.find(
      p => p.id === connection.to
    )
...
function createPin(pinData) {
  ...
  const btndelete = document.createElementNS('http://www.w3.org/2000/svg', 'text')
  ...
  pin.setAttribute('d', 'M32,0C18.745,0,8,10.745,8,24c0,5.678,2.502,10.671,5.271,15l17.097,24.156C30.743,63.686,31.352,64,32,64 s1.257-0.314,1.632-0.844L50.729,39C53.375,35.438,56,29.678,56,24C56,10.745,45.255,0,32,0z M32,38c-7.732,0-14-6.268-14-14 s6.268-14,14-14s14,6.268,14,14S39.732,38,32,38z')
  pin.setAttribute('fill', 'red')
  pin.setAttribute('transform', `translate(${x}, ${y}) scale(0.15) translate(-32, -64)`)
